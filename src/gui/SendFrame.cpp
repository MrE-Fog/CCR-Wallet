...
void SendFrame::parsePaymentRequest(QString _request) {
    if(_request.startsWith("concord://", Qt::CaseInsensitive))
    {
       _request.replace(0, 7, "concord:");
    }
    if(!_request.startsWith("concord:", Qt::CaseInsensitive)) {
      QCoreApplication::postEvent(&MainWindow::instance(), new ShowMessageEvent(tr("Payment request should start with concord:"), QtCriticalMsg));
      return;
    }
